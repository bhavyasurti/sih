...
@router.post("/upload")
async def upload_audit_file(file: UploadFile = File(...), db: Session = Depends(get_db)):
...
async def analyze_audit(audit_id: int, options: AnalysisOptions | None = None, db: Session = Depends(get_db)):
    repository = AuditRepository(db)
    audit = repository.get_audit(audit_id)
    if audit is None:
        raise HTTPException(status_code=404, detail="Audit not found")
    ...
    ai_service = AIService(enabled=(options.ai_enabled if options else True))

    ai_result = ai_service.normalize_configuration(config_text, vendor, deterministic)
    merged = deterministic
    if ai_result.get('status') == 'used' and ai_result.get('normalized'):
        merged = ai_service.merge_normalized_models(deterministic, ai_result['normalized'])
    elif ai_result.get('status') in {'fallback', 'unavailable'}:
        merged = deterministic
...
async def evaluate_compliance(audit_id: int, request: ComplianceRequest, db: Session = Depends(get_db)):
    repository = AuditRepository(db)
    audit = repository.get_audit(audit_id)
    if audit is None:
        raise HTTPException(status_code=404, detail="Audit not found")
...
async def list_generated_reports(db: Session = Depends(get_db)):
    repository = AuditRepository(db)
